...
def build_frames_with_parallel(data_dir: Path):
    ...
    all_frames_are_rendered = all(png_path.exists() for png_path in png_paths)
    if not ONLY_ANIMATE or not all_frames_are_rendered:
        npy_paths = [
            npy_dir / f"slice_{frame_index:05d}.npy" for frame_index in range(len(data_paths))
        ]
        grouped_args = [
            (str(data_path), str(npy_path)) for data_path, npy_path in zip(data_paths, npy_paths)
        ]
        ...
        frame_titles = [
            f"{data_path.name}: t = {t:.3f}" for data_path, t in zip(data_paths, sim_times)
        ]
        render_args = [
