...
def plot_profiles(data_dir: Path):
    ...
    grouped_args = [(str(path), ) for path in data_paths]
    ...
    sim_times = [results[0] for results in results if results is not None]
    x_centers = next(
        (results[1] for results in results if results is not None),
        None,
    )
    profiles = [results[2] for results in results if results is not None]
